...
public class RunTestBatch {
    ...
    public static void main(String[] args) {
        ...
        String testname = strArray[strArray.length - 1];
        String caseParent = delayValue + File.separator + appPackage + File.separator + testname;
        output = caseParent + File.separator + "out.txt";
        ...
        String pullSrc = "/data/user/0/" + appPackage + "/files/dconfig.properties";
        String pullTgt = "/home/dongzhen/AndroidAppInstrument/" + caseParent;
        pullFile(pullSrc,pullTgt);
        sleepAfterAdb();
        String filePath = pullTgt + File.separator + "dconfig.properties";
        ...
        String pullFtgt = "/home/dongzhen/AndroidAppInstrument/" + caseParent;
        pullFile(pullFsrc,pullFtgt);
        sleepAfterAdb();
        String frameworkDelays = pullFtgt + File.separator + "config.properties";
        ...
        for(Map<String,String> config : allConfigs){
            ...
            String pushSrc = filePath;
            String pushTgt = "/data/user/0/" + appPackage + "/files";
...
//        FileUtil.filePrintln("output.txt","The number of delay combinations in framework: "+fConfigs.size());
        for(Map<String,String> fconfig : fConfigs){
            ...
            String pushFsrc = frameworkDelays;
        ...
        deleteConfig("/data/user/0/" + appPackage + "/" + "files/config.properties");
        deleteConfig("/data/user/0/" + appPackage + "/" + "files/dconfig.properties");
    }
    ...
    private static void deleteConfig(String file){
        String deleteCmd = "adb -s emulator-5554 shell rm " + file;
        try{
            Process p = Runtime.getRuntime().exec(deleteCmd);
            BufferedReader reader =
                    new BufferedReader(new InputStreamReader(p.getInputStream()));
